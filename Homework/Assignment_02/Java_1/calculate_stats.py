...
with open("visit_nytimes.csv", "r", encoding="UTF-8") as f:
    data = pd.read_csv(f, header=0)
    total_urls_extracted = data["# Of Outgoing Links"].sum()
    less_1KB = data[data["Download Size"] < 1024].shape[0]
    less_10KB = data[(1024 <= data["Download Size"]) & (data["Download Size"] < 10 * 1024)].shape[0]
    less_100KB = data[(10 * 1024 <= data["Download Size"]) & (data["Download Size"] < 100 * 1024)].shape[0]
    less_1mb = data[(100 * 1024 <= data["Download Size"]) & (data["Download Size"] < 1024 * 1024)].shape[0]
    greater_1mb = data[1024 * 1024 <= data["Download Size"]].shape[0]
...
with open("CrawlReport_nytimes.txt", "w") as f:
    f.write(f"Name: Meerza Ahmed\n")
    f.write(f"USC ID:  8203-5365-10\n")
    f.write(f"News site crawled: https://www.nytimes.com/\n")
